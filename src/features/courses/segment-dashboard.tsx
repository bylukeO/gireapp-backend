...
async function getDashboardData(userId: string) {
  const [enrolments, badgeCount, user] = await Promise.all([
    ...
    prisma.user.findUnique({
      where: { id: userId },
      select: { points: true, academicLevel: true, department: true, moodTheme: true },
    }),
  ]);
...
export async function SegmentDashboard({ config }: { config: SegmentConfig }) {
  ...
  if (session.user.academicLevel !== config.level) {
    redirect('/dashboard');
  }
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatsCard
          ...
          value={data.enrolments.length.toString()}
          color="indigo"
        />
        <StatsCard
          icon={Trophy}
        ...
        <StatsCard
          ...
          value={
            data.enrolments.length > 0
              ? formatProgress(
                  data.enrolments.reduce((sum, e) => sum + e.progress, 0) /
        ...
        {data.enrolments.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {data.enrolments.map((enrolment) => {
              ...
              const progressPercent = Math.round(enrolment.progress * 100);
                    ...
                    <CardContent className="p-5 space-y-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between text-caption text-muted-foreground">
                          <span>
                            {enrolment.course.modules.length} modules •{' '}
